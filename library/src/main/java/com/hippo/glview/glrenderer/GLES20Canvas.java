...
public class GLES20Canvas implements GLCanvas {
    ...
    private static final String TEXTURE_COLORED_FRAGMENT_SHADER = ""
            + "precision mediump float;\n"
            + "varying vec2 vTextureCoord;\n"
            + "uniform float " + ALPHA_UNIFORM + ";\n"
            + "uniform sampler2D " + TEXTURE_SAMPLER_UNIFORM + ";\n"
    ...
    // GL program switch
    private boolean mIsAutoColorMode;

    // GL programs
    private final int mDrawProgram;
    private final int mTextureProgram;
    private final int mTextureColoredProgram;
    private final int mMeshProgram;
    private final int mMeshColoredProgram;
    ...
    public GLES20Canvas() {
        ...
        int textureColoredFragmentShader = loadShader(GLES20.GL_FRAGMENT_SHADER, TEXTURE_COLORED_FRAGMENT_SHADER);
        ...
        mTextureColoredProgram = assembleProgram(textureVertexShader, textureColoredFragmentShader, mTextureParameters);
        mMeshProgram = assembleProgram(meshVertexShader, textureFragmentShader, mMeshParameters);
        mMeshColoredProgram = assembleProgram(meshVertexShader, textureColoredFragmentShader, mMeshParameters);

        GLES20.glBlendFunc(GLES20.GL_ONE, GLES20.GL_ONE_MINUS_SRC_ALPHA);
        checkError();
    }

    public void setAutoColorMode(boolean isAutoColorMode) {
        mIsAutoColorMode = isAutoColorMode;
    }
    ...
    private void drawTextureRect(BasicTexture texture, float[] textureMatrix, RectF target) {
        ...
        prepareTexture(texture, mIsAutoColorMode ? mTextureColoredProgram : mTextureProgram, params);
    ...
    public void drawMesh(BasicTexture texture, int x, int y, int xyBuffer, int uvBuffer,
        ...
        prepareTexture(texture, mIsAutoColorMode ? mMeshColoredProgram : mMeshProgram, mMeshParameters);
